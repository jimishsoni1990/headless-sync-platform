...
/**
 * Unit tests for PgsqlOutboxConnection exception-boundary translation.
 *
 * DECISION E (v1.5) requires that DatabaseException from the shared layer is
 * translated to OutboxWriteException at the relay boundary so consumers never
 ...
 */
final class PgsqlOutboxConnectionTest extends TestCase
{
    private PgsqlOutboxConnection $conn;

    protected function setUp(): void
    {
        $this->conn = new PgsqlOutboxConnection(new FailingPostgresDatabaseConnection());
    }

    public function test_execute_translates_database_exception_to_outbox_write_exception(): void
    {
        $this->expectException(OutboxWriteException::class);
        $this->conn->execute('SELECT 1');
    }

    public function test_execute_wraps_exception_as_outbox_write_not_database_exception(): void
    {
        $caught = null;
        try {
            $this->conn->execute('SELECT 1');
        ...
        self::assertInstanceOf(OutboxWriteException::class, $caught,
            'execute() must throw OutboxWriteException, not raw DatabaseException — DECISION E');
        self::assertNotInstanceOf(DatabaseException::class, $caught,
            'DatabaseException must not escape the outbox boundary — DECISION E');
    }

    public function test_query_translates_database_exception_to_outbox_write_exception(): void
    {
        $this->expectException(OutboxWriteException::class);
    ...
    public function test_invalid_handle_translates_to_outbox_write_exception(): void
    {
        $this->expectException(OutboxWriteException::class);
        // Passing a non-handle value triggers DatabaseException in PostgresDatabaseConnection
        // constructor; PgsqlOutboxConnection must translate it.
        new PgsqlOutboxConnection('not-a-connection-handle');
    }

    public function test_invalid_handle_wraps_exception_as_outbox_write_not_database_exception(): void
    {
        $caught = null;
        try {
            new PgsqlOutboxConnection('not-a-connection-handle');
        ...
        self::assertInstanceOf(OutboxWriteException::class, $caught,
            'Constructor must throw OutboxWriteException, not raw DatabaseException — DECISION E');
        self::assertNotInstanceOf(DatabaseException::class, $caught,
            'DatabaseException must not escape the outbox boundary — DECISION E');
    }
}
