...
/**
 * Validates raw WordPress post data before PostExtractor builds a PostSourceModel.
 *
 * Required fields: ID (numeric > 0), post_type must be 'post'.
 * Structural checks: post_date_gmt and post_modified_gmt must be absent,
 * empty, the zero-datetime sentinel, or a parseable datetime string.
 *
 * Throws ValidationException immediately on the first structural failure
 * (fail-fast); collects all missing-field violations together so callers
 * see the full picture in one throw.
 */
final class PostValidator
{
    ...
    public function validate(array $rawPost): void
    {
        ...
        if (isset($rawPost['post_type']) && $rawPost['post_type'] !== 'post') {
            $violations[] = "Field 'post_type' must be 'post'; got '{$rawPost['post_type']}'.";
        }

        if (! empty($violations)) {
            throw new ValidationException(
                'Post source data failed validation: ' . implode(' ', $violations),
                $violations,
            );
        }
