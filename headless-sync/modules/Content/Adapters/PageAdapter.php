...
final class PageAdapter implements AdapterInterface
{
    ...
    /**
     * Soft-delete the content.pages row for this aggregate (DECISION I).
     *
    ...
    public function tombstone(string $aggregateType, string $aggregateId, EventInterface $event): void
    {
        ...
        try {
            $this->db->execute(
                'UPDATE content.pages SET deleted_at = $1::timestamptz WHERE source_post_id = $2',
        ...
        $this->db->execute(
            'INSERT INTO content.pages
                (id, source_post_id, source_entity_type, slug, title, content, status,
                 parent_id, menu_order, featured_media_id, published_at, updated_at, deleted_at,
