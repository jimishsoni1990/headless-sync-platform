...
| Field | Value |
|---|---|
| **Status** | **Resolved — architect ruling 2026-06-23; AMENDED in part by DECISION AJ (v1.42, 2026-09-14)** |
...
> **⚠ AMENDED IN PART BY DECISION AJ (v1.42, 2026-09-14).** The **identity column** above is
> superseded: the term is referenced by its stable source identity, so the relationship shape is
> `(entity_id UUID, source_term_id BIGINT)` composite PK. Referencing the term's *projection* id
> required `content.taxonomies` to have materialised first, which under at-least-once, non-FIFO
> delivery it often has not — and the resulting missing relationships proved unrepairable, because
> the post's own checksum never moves and both replay and reconciliation compare it (Finding 004).
> **The rest of this ruling stands verbatim and is strengthened, not weakened:** still a pure
> relationship table, composite PK only, no timestamps, no checksum, no metadata, no surrogate id.
> See DECISION AJ.
...
(the existing PK) and `(taxonomy_id, entity_id)`. [**DECISION AJ (v1.42) re-keys this table on the
source term identity; both orderings survive as `(entity_id, source_term_id)` — the PK — and
`(source_term_id, entity_id)`. The two access paths this paragraph names are unchanged; only the
column that identifies the term changed.**] The pre-existing single-column
...
> **⚠ AMENDED IN PART BY DECISION AJ (v1.42, 2026-09-14).** "Keep their columns, keys and
> constraints" no longer holds for `content.entity_taxonomies`: its term-identity column becomes
> `source_term_id BIGINT`, so the PK is `(entity_id, source_term_id)` and the reverse index
> `(source_term_id, entity_id)`. This decision's own model is otherwise untouched and is the reason
> the amendment is safe — one shared `content.taxonomies` owning `taxonomy_type`, discrimination
> still mandatory on every read, still a pure relationship table, still no new persistence.
> `content.taxonomies` itself is entirely unchanged.
...
### DECISION AJ — Content Taxonomy Relationships Key on the Source Term Identity

**Status: Accepted (architect ruling 2026-09-14). Amends FLAG-P1AS4-1 (v1.8) and DECISION AA's
"Explicitly unchanged" clause (v1.33). Resolves Finding 004.**

**This is a narrow amendment to a frozen shape, prompted by a defect proven in production. It is
recorded as an amendment precisely because the previous shape was frozen — it was not, and could
not have been, changed silently by any earlier ruling.**

**Previous assumption.** FLAG-P1AS4-1 (architect ruling 2026-06-23) froze
`content.entity_taxonomies` as exactly `(entity_id UUID, taxonomy_id UUID)`, the term referenced by
its **projection** identity. DECISION AA (v1.33) upheld that shape verbatim and built the
`(taxonomy_id, entity_id)` reverse index on it. The unexamined assumption underneath both was that
the term's projection row would exist by the time a post's relationship was written.

**Proven failure.** It does not. Under at-least-once, non-FIFO delivery a post routinely projects
before its terms, and on the installation that surfaced this **every** post projected before
**every** category: `content.post.updated` at 07:12:06, `content.category.updated` at 07:12:06–07,
posts materialised 07:12:09–10, categories 07:12:10. `PostAdapter` resolved each source term id to
a `content.taxonomies.id`, found nothing, and silently wrote no relationship — the documented
expectation being that the link would appear "when the category syncs". **Nothing implements that:
`PostAdapter` is the only writer of the table, and no taxonomy-side relationship writer exists.**

What made it permanent rather than merely late is write suppression. A post's category ids are
inside its canonical checksum, so once WordPress membership settles the recomputed checksum equals
the stored one, DECISION 3 correctly suppresses the write, and the relationship rewrite never runs
again. `ReconciliationService::classifyForward()` compares that same checksum and `modifiedAt`, so
drift detection never saw a gap either. Delivery served `post_count: 6` from
`GET /hsp/v1/categories/etf` while `GET /hsp/v1/posts?category=etf` returned `[]`, with **zero**
rows in `content.entity_taxonomies` platform-wide and no error logged anywhere.

**Ruling.** `content.entity_taxonomies` references the term by its **stable source identity**:

```
entity_id       UUID    NOT NULL
source_term_id  BIGINT  NOT NULL
PRIMARY KEY (entity_id, source_term_id)
index (source_term_id, entity_id)   -- reverse access path, replacing (taxonomy_id, entity_id)
```

A relationship row is therefore a **pure function of the owning post's own state**: complete the
moment the post projects, in any arrival order, and never dependent on `content.taxonomies.id`
having been materialised. The term row is joined at read time on `source_term_id`, which carries
`uq_content_taxonomies_source_term_id` and is globally unique across WordPress taxonomies. The
delivery-side invariant this establishes:
...
**(AJ-1) Relationship-state comparison joins the suppress decision.** The canonical checksum
witnesses the `content.posts` row **only**, and this defect proved that is not the whole projection
for this aggregate. `PostAdapter` therefore suppresses the projection write only when the stored
row's checksum matches **and** the stored relationship set is **exactly equal** — as a set,
order-insensitive and duplicate-safe — to the canonical term set. Cardinality equality is **not**
sufficient and must not be used: `[10, 20]` and `[10, 30]` have the same count and are not the same
projection. The comparison is bounded to one aggregate and rides the `entity_id`-prefixed primary
key; it must never become a per-term lookup (N+1). This is what lets an installation damaged before
this amendment heal through the ordinary path — replay re-emits, the adapter observes the
relationship set is wrong, and rewrites it — with **no second repair path**.

**(AJ-2) Deliberately unchanged.** `content.entity_taxonomies` remains a **pure relationship
table**: composite primary key and nothing else — no timestamps, no checksum, no relationship
metadata, no surrogate id (the substance of FLAG-P1AS4-1, which this amendment narrows to the
identity column alone). **No** separate category/tag relationship tables; categories, tags and
every future Content taxonomy continue to share `content.taxonomies`, told apart by
`taxonomy_type`, which `content.taxonomies` still owns (DECISION AA). **Taxonomy discrimination
remains mandatory** on every read of the shared projection — `taxonomy_type = 'category'` for the
category filter, `'post_tag'` for tag expansion and filtering — and slug alone is never sufficient.
**No foreign keys** (ADR-013 / AG-7 reasoning: either side may legitimately arrive later).
`source_term_id` is an **internal projection identity and never a public Delivery API identifier**
(ADR-040 internal-column exclusion stands); consumers continue to filter by **slug**. WordPress
remains source of truth, PostgreSQL remains the delivery projection, and no WordPress read enters
the consumer request path.

**(AJ-3) Migration.** `0009_align_content_entity_taxonomies_to_source_term_id` (`content/pgsql`)
translates existing relationship rows in place through `content.taxonomies`, **preserving every
link whose term resolves**; rows referencing a taxonomy identity that cannot be translated are
dangling by definition and are removed rather than given a manufactured source identity. It
replaces the reverse index and leaves **one** authoritative relationship identity — no transitional
dual-key state, no additional relationship table, and no obsolete UUID index left behind.

**(AJ-4) Scope.** This amendment solves **ordering and convergence only**. It changes no response
shape, no public identifier, no filter contract, no OpenAPI document, no module boundary, and adds
no persistence subsystem. It does not touch `post_count`, whose semantics (WordPress's own
`wp_term_taxonomy.count`, projected verbatim) were investigated and found correct and consistent
with the archive. `commerce.entity_taxonomies` is untouched — it already keys on `source_term_id`.

**Relationship to AG-7 — supporting precedent, not retroactive authority.** DECISION AG (AG-7)
established source-identity references for asynchronously projected **Commerce** relationships, and
`0004_create_commerce_entity_taxonomies` recorded in its own header that it "deliberately differs
from `content.entity_taxonomies`, which stores the projection UUID". That is corroborating evidence
that the design here is the one consistent with HSP's convergence guarantees, and the Commerce
table on the affected installation was fully populated from the same backfill in the same ordering
while Content's was empty. **It is not a claim that AG-7 had already amended Content.** It had not:
Content's UUID relationship identity stayed frozen, the real implementation exposed a convergence
...
> **Note (v1.8 — P1A-S4 delivery):** `content.pages`, `content.posts`, `content.taxonomies`, and `content.entity_taxonomies` migrations were delivered in P1A-S4. All timestamp columns use `TIMESTAMPTZ`; all checksum columns use `VARCHAR(64)`. `content.entity_taxonomies` is a pure join table — (entity_id UUID, taxonomy_id UUID) composite PK only (FLAG-P1AS4-1). **(v1.42: the term-identity column is amended to `source_term_id BIGINT` by DECISION AJ — still a pure join table, composite PK only.)** The freeze check for all `content.*` tables occurs at the Phase 1A DoD gate (end-to-end validation in P1A-S6) per the v1.2 rule. `content.media` remains OUT of MVP scope (Phase 1B).
...
| `content.taxonomies` + `content.entity_taxonomies` (shared taxonomy projection) | **One shared taxonomy projection per owning DOMAIN**, told apart by `taxonomy_type` — `category`, `post_tag` and every future **Content** taxonomy live in `content.taxonomies`; a new Content taxonomy is **new data, not a new table and not a migration**. **No table-per-taxonomy** and **no platform-wide taxonomy table** (Commerce gets its own `commerce.*` projection model, designed with the Commerce module — never `content.taxonomies` merely because WordPress stores it via the taxonomy API). `content.entity_taxonomies` stays a **pure join table** (composite PK only — P1A-S4 upheld on shape; its term-identity COLUMN is amended to `source_term_id` by DECISION AJ v1.42, see the row below). **Query rule:** every read of the shared table identifies BOTH taxonomy type and term identity; a read keyed on the globally-unique `source_term_id` is exempt. Enforced by `tests/Unit/Content/TaxonomyQueryRuleTest.php`. **Index deltas (migration `0008_align_content_taxonomy_indexes`):** `+ (taxonomy_type, slug)`, `+ entity_taxonomies (taxonomy_id, entity_id)`, `− (slug)`, `− (taxonomy_type)`, `− entity_taxonomies (taxonomy_id)`; `(taxonomy_type, parent_id)` deliberately **deferred** until a delivery query predicates on parent. **No column, key, constraint, contract, event, handle or persistence change.** No partitioning or table-per-taxonomy optimisation without profiling plus a future ruling. | DECISION AA (v1.33); FLAG-TAXSCHEMA-1; P1A-S4 join-table ruling; Rule 2 / Rule 5 |
| `content.entity_taxonomies` (relationship identity) | **AMENDS FLAG-P1AS4-1 and DECISION AA.** The term is referenced by its **stable source identity**, not its projection UUID: `entity_id UUID NOT NULL`, `source_term_id BIGINT NOT NULL`, `PRIMARY KEY (entity_id, source_term_id)`, reverse index `(source_term_id, entity_id)` **replacing** `(taxonomy_id, entity_id)`; `taxonomy_id` and its index are **dropped** — one authoritative relationship identity, no dual-key transition, no second relationship table. **Why:** keyed on the projection UUID, a relationship could only be written once `content.taxonomies` had materialised; under at-least-once, non-FIFO delivery a post routinely projects first, the write silently produced nothing, and the gap was **permanent** because the post's own checksum never moves and both replay and reconciliation compare it (Finding 004 — `post_count: 6` beside an empty `?category=` archive, zero relationship rows platform-wide). **Still a pure relationship table** — composite PK only, no timestamps, no checksum, no metadata, no surrogate id, no FKs. **Migration `0009_align_content_entity_taxonomies_to_source_term_id`** translates existing rows through `content.taxonomies`, preserving every link whose term resolves and removing untranslatable dangling rows rather than manufacturing a source identity. **`content.taxonomies` is unchanged**, still owns `taxonomy_type`, and discrimination remains mandatory on every read of the shared projection. `source_term_id` is **internal** (ADR-040) — consumers still filter by slug. Adapter write suppression must additionally compare the persisted relationship set to the canonical set for **exact set equality**; **count equality is explicitly insufficient** (AJ-1). | DECISION AJ (v1.42); Finding 004; FLAG-P1AS4-1 as amended; DECISION AA as amended |
